...
import kr.co.auth.SNSLogin;
import kr.co.auth.SnsValue;
import kr.co.constant.SnsType;
import kr.co.domain.User;
import kr.co.dto.LoginDTO;
import kr.co.interceptor.SessionNames;
...
//	}
	...
	public String snsLoginCallback(
		...
		// 3. DB 해당 유저가 존재하는 체크 (Sns컬럼, SnsId 컬럼 추가)
//		if (user == null) {
		model.addAttribute("result", "존재하지 않는 사용자입니다. 가입해 주세요.");
			//미존재시 가입페이지로!!
//		} else {
...
//		}
	...
	public String logout(HttpSession session, 
		...
		if (loginCookie != null) {
			...
			service.keepLogin(user.getU_id(), session.getId(), new Date());
		}
	...
	public void loginPost(LoginDTO dto, Model model, HttpSession session) throws Exception {
		...
		try {
			...
			if (user != null) {
				...
				service.keepLogin(user.getU_id(), session.getId(), expire);
