...
public enum SnsUrlGroup {
	...
	private SnsUrlGroup(SnsUrlType CLIENT_ID, SnsUrlType CLIENT_SECRET, SnsUrlType REDIRECT_URL,
			SnsUrlType ACCESS_TOKEN, SnsUrlType AUTHORIZE, HttpMethod METHOD, SnsUrlType RESOURCE_URL) {
	...
	public String getSnsUniqueId (String code) throws IOException {
		...
		ResponseEntity<String> response = rt.exchange(accessTokenUrl, METHOD, kakaoTokenRequest, String.class);
		...
		ResponseEntity<String> response2 = rt2.exchange(resourceUrl, METHOD, kakaoProfileRequest2,
				String.class);
